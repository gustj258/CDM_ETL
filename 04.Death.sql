...
select * into death_mapping from * cohort_cdm.CONDITION_MAPPINGTABLE
insert (KCDCODE, NAME, CONCEPT_ID, CONCEPT_NAME) select  'A00-A09', 'infectious enteritis', 4134887, 'Infectious disease of digestive tract'  FROM dual
insert (KCDCODE, NAME, CONCEPT_ID, CONCEPT_NAME) select  'A15-A19', 'tuberculosis', 434557, 'Tuberculosis'  FROM dual
...
SELECT a.person_id AS PERSON_ID,
	to_char -DATEPART( * INTERVAL FROM dual '1' EXTRACT(DAY FROM 5)(DD, NUMTODSINTERVAL('1', 'MONTH') +to_char (a.dth_ym + '01' ,23)) +INTERVAL '1' MONTH +to_char (a.dth_ym + '01' ,23), 23) AS DEATH_DATE,
...
FROM bigdata.NHIS_JK a left join #death_mapping b
...
INSERT INTO DEATH@cohort_cdm (person_id, death_date, death_type_concept_id, cause_concept_id, 
cause_source_value, cause_source_concept_id)
SELECT a.person_id AS PERSON_ID,
