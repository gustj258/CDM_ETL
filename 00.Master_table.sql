...
***************************************/  
CREATE TABLE MASTER@cohort_cdm (
	master_seq	  NUMBER CONSTRAINT SEQ_MASTER_master_seq PRIMARY KEY,
	source_table	CHAR(3)	NOT NULL, -- 30T, 40T, 60T는 130, 140, 160. 검진은 'GJT'
	person_id		NUMBER	NOT NULL, -- 모두
	key_seq			NUMBER	NULL, -- 30T, 40T, 60T
	seq_no			NUMBER NULL, -- 30T, 40T, 60T
	hchk_year		CHAR(4)	NULL, -- 검진	
);

-- Generate ID using sequence and trigger
CREATE SEQUENCE SEQ_MASTER_seq@cohort_cdm START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER SEQ_MASTER_seq_tr@cohort_cdm
 BEFORE INSERT ON SEQ_MASTER@cohort_cdm FOR EACH ROW
 WHEN (NEW.master_seq IS NULL)
BEGIN
 SELECT SEQ_MASTER_seq@cohort_cdm NEXTVAL INTO :NEW.master_seq FROM DUAL;
END;
/
-- 607738697
...
-- 2) 데이터 입력
INSERT INTO SEQ_MASTER@cohort_cdm
...
FROM @bigdata.@NHIS_40T a, @bigdata.@NHIS_20T b
WHERE a.key_seq=b.key_seq
...
FROM @bigdata.@NHIS_60T a, @bigdata.@NHIS_20T b
WHERE a.key_seq=b.key_seq
