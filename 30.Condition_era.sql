...
***************************************/ 
CREATE TABLE cohort_cdm.CONDITION_ERA  (
...
CREATE SEQUENCE cohort_cdm.CONDITION_ERA_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER cohort_cdm.CONDITION_ERA_seq_tr
 BEFORE INSERT ON cohort_cdm.CONDITION_ERA FOR EACH ROW
 WHEN (NEW.condition_era_id IS NULL)
BEGIN
 SELECT cohort_cdm.CONDITION_ERA_seq.NEXTVAL INTO :NEW.condition_era_id FROM DUAL;
END;
/ 
...
SELECT
	...
	COALESCE(NULLIF(condition_end_date,NULL), interval '31'  day + condition_start_date) AS condition_end_date
...
(
	SELECT
		...
		, event_date
		, event_type
		, MAX(start_ordinal) OVER FROM dual (PARTITION BY person_id, condition_concept_id ORDER BY event_date, event_type ROWS UNBOUNDED PRECEDING) AS start_ordinal -- this pulls the current START down from the prior rows so that the NULLs from the END DATES will contain a value we can compare with 
	...
	(
		...
		FROM cteConditionTarget UNION ALL
	
		-- pad the end dates by 30 to allow a grace period for overlapping ranges.
		SELECT
			person_id
				, condition_concept_id
			, interval '30'  day +condition_end_date 
			, 1 AS event_type
			, NULL
...
INSERT INTO cohort_cdm.condition_era(person_id, condition_concept_id, condition_era_start_date, condition_era_end_date, condition_occurrence_count)
SELECT
	person_id
	, condition_concept_id
